...
<Tabs items={["Windows", "Linux", "MacOS"]}>
  <Tab>
    ...
    "C:\AtomicRedTeam\invoke-atomicredteam\Invoke-AtomicRedTeam.psd1" -Force ```
  </Tab>
  <Tab>
    ```powershell Import-Module
...
<Tabs items={["Windows", "Linux", "MacOS"]}>
  <Tab>```powershell notepad $profile ```</Tab>
  <Tab>```powershell nano $profile ```</Tab>
  <Tab>```powershell open -a TextEdit $profile ```</Tab>
</Tabs>
